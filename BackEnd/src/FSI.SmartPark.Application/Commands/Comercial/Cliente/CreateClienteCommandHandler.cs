    ...
    : IRequestHandler<CreateClienteCommand, ClienteResponseDto>
{
    ...
    public async Task<ClienteResponseDto> Handle(CreateClienteCommand request, CancellationToken ct)
    {
        ...
        Cliente entidade = request.Documento.Length == 11
            ? new Cliente(request.Nome, new FSI.SmartPark.Domain.ValueObjects.Cpf(request.Documento), request.IsMensalista, request.EmpresaId)
            : new Cliente(request.Nome, new FSI.SmartPark.Domain.ValueObjects.Cnpj(request.Documento), request.IsMensalista, request.EmpresaId);
